...
function splitChunks(s: string): Chunk[] {
  ...
  while (s !== '') {
    ...
    const buf = s.substring(0, size + 12);
    s = s.substring(size + 12); // delete this chunk
    // read chunk data
    const type = buf.substring(4, 8);
    let data = "";
    if (size > 0) {
      data = buf.substring(8, size+8);
    }
    chunks.push({
      type: type,
      data: data,
...
function joinChunks(chunks: Chunk[]): string {
  ...
  for (let chunk of chunks) {
    let buf = itos(chunk.data.length, 4);
    buf += chunk.type;
    buf += chunk.data;
    buf += itos(crc32(chunk.type+chunk.data), 4);
    s += buf;
  }
  return s;
}

function itos(v: number, size: number): string {
  var a = [];
  var t = size - 1;
  while (t >= 0) {
    var c = v & 0xFF;
