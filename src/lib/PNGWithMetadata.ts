...
export class PNGWithMetadata {
  ...
  constructor(filename: string, metadata: { [key: string]: any }, img: HTMLCanvasElement | HTMLImageElement | ImageBitmap | string) {
    ...
    if (img instanceof HTMLCanvasElement) {
      this.imageBytes = base64ToBytes(img.toDataURL('image/png').substring(DATA_PNG.length));
    } else if (img instanceof HTMLImageElement) {
      this.imageBytes = base64ToBytes(img.src.substring(DATA_PNG.length));
      ...
      this.imageBytes = base64ToBytes(canvas.toDataURL('image/png').substring(DATA_PNG.length));
    ...
    } else if (img.startsWith(PNG_SIG)) {
  ...
  dataURL(): string {
    return DATA_PNG + bytesToBase64(writeMetadata(
      this.imageBytes,
      this.metadata,
    ));
  }
...
function writeMetadata(buf: Uint8Array, metadata: { [key: string]: any }): Uint8Array {
  const chunks = splitChunks(buf);
  if (Object.keys(metadata).length > 0) {
    const i = chunks.findIndex(isMetadataChunk);
    const chunk = {
      type: ChunkType.zTXt,
      data: pako.deflate(JSON.stringify(metadata)),
    };
    if (i === -1) {
...
function isMetadataChunk(c: Chunk): boolean {
  return c.type === ChunkType.zTXt;
}

function splitChunks(buf: Uint8Array): Chunk[] {
  buf = buf.subarray(PNG_SIG.length); // chomp sig
  const  chunks = [];
  // read chunk list
  while (buf.length) {
    // read chunk size
    let size = stoi(String.fromCodePoint(...buf.subarray(0, 4)));
    if (size < 0) {
    ...
    const chunkBuf = buf.subarray(0, size + 12);
    buf = buf.subarray(size + 12); // delete this chunk
    // read chunk data
    const type = String.fromCodePoint(...chunkBuf.subarray(4, 8));
    let data = new Uint8Array();
    if (size > 0) {
      data = chunkBuf.subarray(8, size+8);
    }
    chunks.push({
...
function joinChunks(chunks: Chunk[]): Uint8Array {
  ...
  for (let chunk of chunks) {
    if (Object.values(ChunkType).includes(chunk.type)) {
      const buf = new Uint8Array(8 + chunk.type.length + chunk.data.length);
      buf.set(itos(chunk.data.length, 4));
      buf.set(Uint8Array.from(chunk.type, c => c.codePointAt(0) || 0), 4);
      buf.set(chunk.data, 4+chunk.type.length);
      buf.set(itos(crc32(buf.subarray(4, -4)), 4), 4+chunk.type.length+chunk.data.length);
      bufs.push(buf);
      len += buf.length;
    }
  }
  const result = new Uint8Array(PNG_SIG.length+len);
  result.set(Uint8Array.from(PNG_SIG, c => c.codePointAt(0) || 0));
...
function itos(v: number, size: number): Uint8Array {
...
function crc32(buf: Uint8Array): number {
  ...
  for (let i = 0; i < buf.length; i++) {
    crc = (crc >>> 8) ^ hexTable[(crc ^ buf[i]) & 0xFF];
  }
  return (crc ^ (-1)) >>> 0;
}
