...
export default class PNGWithMetadata {
  ...
  constructor(
    ...
    if (img instanceof HTMLCanvasElement) {
      this.imageBytes = base64ToBytes(
        img.toDataURL("image/png").substring(DATA_PNG.length),
      );
  ...
  dataURL(): string {
    return (
      DATA_PNG + bytesToBase64(writeMetadata(this.imageBytes, this.metadata))
    );
  }
...
function joinChunks(chunks: Chunk[]): Uint8Array {
  ...
  for (let chunk of chunks) {
    if (Object.values(ChunkType).includes(chunk.type)) {
      ...
      buf.set(
        Uint8Array.from(chunk.type, (c) => c.codePointAt(0) || 0),
        4,
      );
      buf.set(chunk.data, 4 + chunk.type.length);
      buf.set(
        itos(crc32(buf.subarray(4, -4)), 4),
        4 + chunk.type.length + chunk.data.length,
      );
      bufs.push(buf);
      len += buf.length;
    }
